...
public class JwtTokenUtil implements Serializable {
    ...
    static final String CLAIM_KEY_CREATED = "data";
    ...
    private Boolean ignoreTokenExpiration(String token) {
        ...
        return false; //(AUDIENCE_TABLET.equals(audience) || AUDIENCE_MOBILE.equals(audience));
    }

    public String generateToken(JwtUser jwtUser) throws JsonProcessingException {
        Map<String, Object> claims = new HashMap<>();
        claims.put(CLAIM_KEY_EMAIL, jwtUser.getUsername());
        claims.put(CLAIM_KEY_CREATED, new Date());
        List<String> auth =jwtUser.getAuthorities().stream().map(role-> role.getAuthority()).collect(Collectors.toList());
        claims.put(CLAIM_KEY_AUTHORITIES, auth);
        claims.put(CLAIM_KEY_IS_ENABLED,jwtUser.isEnabled());
